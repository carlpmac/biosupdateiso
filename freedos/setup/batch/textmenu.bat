...
set cputype=80386
cpulevel > NUL
for %%x in ( 0 1 2 ) do if "%errorlevel%"=="%%x"set cputype=80%errorlevel%86
if "%errorlevel%=="0" set cputype=80186
if "%lang%"=="" set lang=EN
...
REM BBL2011: all SYS code disabled, leave it to SAMCFG.ZIP please!
REM if "%os%"=="37"   iniadd C:\BOOT.INI FREEDOS.BSS %destdrv%\FREEDOS.BSS="FreeDOS"
pushd
cdd %fdosroot%\odin
REM if not exist %destdrv%\FREEDOS.BSS SYS %destdrv% %destdrv% %destdrv%\FREEDOS.BSS >NUL
popd
REM if "%os%"=="37" goto skipsys
goto setup
...
pushd
cdd %fdosroot%\odin
REM if not exist %destdrv%\metakern.sys SYS %destdrv%>NUL
popd
REM if exist %destdrv%\metakern.sys SYS %destdrv% /BOOTONLY /K metakern.sys>NUL
:skipsys
xmssize 5 >NUL
set xms=%errorlevel%
