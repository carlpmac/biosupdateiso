...
REM BBL2011/FIXME: CHECK UIDE/SHSUCDX/SHSUCDRD/SHSUCDHD + MetaKern Multiboot (or SYSLINUX/32/x64 - easier????)
REM 64bit platform doesn't allow 16bit programs, some rewriting is in order..
REM What about Windows's UAC, administrator requirements etc?
...
:docopy
rem BBL2011: Ruxgulo's site has some GPL DOS image extracting software in images
...
if exist %dest%\uide.sys copy /y %dest%\uide.sys %dest%\driver\uide.sys
